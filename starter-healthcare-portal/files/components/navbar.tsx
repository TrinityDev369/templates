      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="text-xl font-bold tracking-tight text-gray-900">
            Medi<span className="text-brand-500">Care</span>{" "}
            <span className="font-normal text-brand-600">Plus</span>
          </span>
        </Link>
        ...
        <div className="space-y-1 px-4 pb-4 pt-3">
          {navLinks.map((link) => (
            <Link
              ...
              className="block rounded-md px-3 py-2 text-base font-medium text-gray-600 transition-colors hover:bg-brand-50 hover:text-brand-700"
            >
              {link.label}
            </Link>
          ))}
